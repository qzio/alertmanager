...
type templateData struct {
	...
	AlertCommonLabelnamesSorted []string
	GroupLabels                 map[string]string
	GroupLabelnamesSorted       []string
}
...
func generateTemplateData(ctx context.Context, as ...*types.Alert) *templateData {
	...
	data := &templateData{
		...
		AlertCommonLabelnamesSorted: []string{},
		GroupLabels:                 map[string]string{},
		GroupLabelnamesSorted:       make([]string, 0, len(groupLabels)),
	}
	...
	for k, v := range groupLabels {
		...
		data.GroupLabelnamesSorted = append(data.GroupLabelnamesSorted, string(k))
	}
	sort.Strings(data.GroupLabelnamesSorted)
	...
	if len(alerts) >= 1 {
		...
		for k, v := range common {
			...
			data.AlertCommonLabelnamesSorted = append(data.AlertCommonLabelnamesSorted, string(k))
		}
	}
	sort.Strings(data.AlertCommonLabelnamesSorted)
...
func (n *Email) Notify(ctx context.Context, as ...*types.Alert) error {
	...
	if err != nil {
		return fmt.Errorf("error executing from template: %s", err)
	}
	addrs, err := mail.ParseAddressList(from)
	if err != nil {
		return fmt.Errorf("error parsing from addresses: %s", err)
	}
	if len(addrs) != 1 {
		return fmt.Errorf("error, must be exactly one from address")
	}
	...
	if err != nil {
		return fmt.Errorf("error executing to template: %s", err)
	}
	addrs, err = mail.ParseAddressList(to)
	if err != nil {
		return fmt.Errorf("error parsing to addresses: %s", err)
	}
	...
	for header, tmpl := range n.conf.Headers {
		...
		if err != nil {
			return fmt.Errorf("error executing %q header template: %s", header, err)
		}
